...
{  
	...
	if(PrimitiveType != 4 || VertexStreamZeroStride != 28)
		matched = false;

	if(matched && (*((unsigned char*)0x0164D0B1)) != 10 || (*((unsigned char*)0x017D5EFB)) != 1)
		matched = false;

	if(PrimitiveCount % 2 != 0)
		matched = false;

	lvert* verts = (lvert*)pVertexStreamZeroData;

	for(unsigned int x = 0; x < PrimitiveCount * 3 && matched; x+= 6) 
	{
		bool matched2 = true;
		for(int y = 0; y < 6; y++) 
		{
			//if(verts[x+y].diffuse != 0xFFF05080 && verts[x+y].diffuse != 0x80405080)
			if(verts[x+y].diffuse == 0xFFFFFFFF)
				matched2 = false;
		}
		double error = 0.0;
		error += fabs(verts[x  ].tu - 0.000000) + fabs(verts[x  ].tv - 0.375000);
		error += fabs(verts[x+1].tu - 0.062500) + fabs(verts[x+1].tv - 0.375000);
		error += fabs(verts[x+2].tu - 0.000000) + fabs(verts[x+2].tv - 0.437500);
		error += fabs(verts[x+3].tu - 0.062500) + fabs(verts[x+3].tv - 0.375000);
		error += fabs(verts[x+4].tu - 0.000000) + fabs(verts[x+4].tv - 0.437500);
		error += fabs(verts[x+5].tu - 0.062500) + fabs(verts[x+5].tv - 0.437500);
		
		if(matched2 && error < 0.000001) 
			verts[x].diffuse = verts[x+1].diffuse = verts[x+2].diffuse = verts[x+3].diffuse = verts[x+4].diffuse = verts[x+5].diffuse &= 0x00FFFFFF;
	}
